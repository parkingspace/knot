...
function Install-Rustup {
	...
	Start-BitsTransfer -Source $downloadUri -Destination $executablePath
	Start-Process -FilePath $executablePath -ArgumentList "-y" -PassThru -Wait -Verb RunAs | Out-Null
...
try {
	...
	# greeting
	Write-Host "Knot Development Environment Setup" -ForegroundColor Magenta
	Write-Host @"
To set up your machine for Knot development, this script will do the following if necessary:
...
"@
	...
	# fin
	Write-Host
	Write-Host "Your machine has been set up for Knot development!" -ForegroundColor Magenta
}
finally {
